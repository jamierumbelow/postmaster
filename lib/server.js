...
exports.Server = (function() {
  ...
  function Server(next, host, port, quiet, store) {
    var STATES,
      _this = this;
    this.host = host != null ? host : 'localhost';
    this.port = port != null ? port : 5666;
    this.quiet = quiet != null ? quiet : false;
    this.store = store != null ? store : false;
    ...
    this.server.on('connection', function(socket) {
      ...
      return socket.on('data', function(data) {
        ...
        return lines.forEach(function(line, index) {
          return _this.handler(socket, line, state);
        });
      });
    });
  }
  ...
  Server.prototype.handler = function(socket, line, state) {
    var STATES, body, email, id, token;
    STATES = Server.STATES;
    ...
    if (token.meaning === 'data-collection' && state.state === STATES.data && line === "") {
      ...
      state.email = this.parser.parseEmail(body);
      state.email.from = email.from;
      state.email.to = email.to;
      id = this.store.add(state.email);
